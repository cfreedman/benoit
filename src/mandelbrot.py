...
def escape(c_point: ComplexPoint, max_iteration: int) -> int:
    loop_point = ComplexPoint(0, 0)
    iter = 0

    while loop_point.length_squared() <= 4 and iter <= max_iteration:
...
def average_escape(grid: GridPoints, max_iteration: int) -> int:
    ...
    for i in range(len(grid.x_grid)):
        for j in range(len(grid.y_grid)):
            ...
            total += escape(c_point, max_iteration)
